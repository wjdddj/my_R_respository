...
surveyCandidates <- function(dataFrame, CaseAboveCutoff = NULL, CaseBelowCutoff = NULL, ControlAboveCutoff = NULL, ControlBelowCutoff = NULL){
  
  if(is.null(CaseAboveCutoff) & is.null(CaseBelowCutoff) & is.null(ControlAboveCutoff) & is.null(ControlBelowCutoff)) {
    cat('please provide a cutoff value. \n')
    z <- NULL
  }
  else{
...
combineRule1 <- function(dataFrame, method = 'case-specific', topCaseFeatures, atLeastCase = 2, topControlFeatures, atLeastControl = 2){
  ...
  if(method == 'case-specific'){
    ...
    caseCall <- data.frame(sampleID = rownames(dataFrame), 
                           predict = as.numeric(apply(trueCaseTable, 1, sum, na.rm = T) >= atLeastCase))
    z <- caseCall
  }
    ...
    controlCall <- data.frame(sampleID = rownames(dataFrame), 
                              predict = as.numeric(apply(trueControlTable, 1, sum, na.rm = T) < atLeastControl))
    z <- controlCall
  }
  z
}
...
############################################################################################################################################################################
# combine two direction of prediction
# caseRule, controlRule are output from combineRule1()
############################################################################################################################################################################
uniPredict <- function(caseRule, controlRule){
  prediction <- caseRule$predict
  unpredictVec <- caseRule$predict - controlRule$predict
  prediction[unpredictVec != 0] <- 'unpredictable'
  posControdictory <- as.numeric(caseRule$predict != controlRule$predict & caseRule$predict == 1)
  z <- data.frame(sampleID = caseRule$sampleID,
                  predict = prediction,
                  posControdictory = posControdictory,
                  caseRule = caseRule$predict,
                  controlRule = controlRule$predict)
}
...
recursiveRules_v1 <- function(dataFrame, 
  ...
  if(method == 'both' & maxRound == 1){
    method == '1 round both directions'
  }
  
  if(method == '1 round both directions'){
    cat('\nperforming 1 round partition on both directions.\n')
    ...
    # select features based on sensitivity and specificity ranks
    ...
    parameters <- data.frame(nRound, topCaseFeatures, atLeastNumber = atLeastCase)
    parameters <- rbind(parameters, data.frame(nRound, topControlFeatures, atLeastNumber = atLeastControl))
    
    # predict 
    diagCaseMatrix <- dataFrame[, match(topCaseFeatures$feature, colnames(dataFrame))]
    caseRule <- combineRule1(diagCaseMatrix, method = 'case-specific', topCaseFeatures = topCaseFeatures, atLeastCase = atLeastCase)
    diagControlMatrix <- dataFrame[, match(topControlFeatures$feature, colnames(dataFrame))]
    controlRule <- combineRule1(diagControlMatrix, method = 'control-specific', topControlFeatures = topControlFeatures, atLeastControl = atLeastControl)
    
    # assign 'unpredictable' to conflictory samples
    predictTable <- uniPredict(caseRule, controlRule)
    numberOfPredictable <- sum(predictTable$predict != 'unpredictable')
    numberOfUnPredictable <- sum(predictTable$predict == 'unpredictable')
    numberOfContradictory <- sum(predictTable$posControdictory == 1)
    cat('node 1, CaseAboveCutoff Features: ', numberOfTopCaseFeatures,
        '; ControlAboveCutoff Features: ', numberOfTopControlFeatures,
        '; sensitivity + specificity > ', specPLUSsensCutoff,
        '; predictable: ', numberOfPredictable,
        '; unpredictable: ', numberOfUnPredictable,
        '; positive contradictory: ', numberOfContradictory, '\n')
    
    roundSummary <- data.frame(nRound, numberOfTopCaseFeatures, numberOfTopControlFeatures, specPLUSsensCutoff, numberOfPredictable, numberOfUnPredictable)
...
predictRecurRules <- function(recursiveRules, newData){
  ...
  else if(method == '1 round both directions'){
    topCaseFeatures <- recursiveRules$parameters[recursiveRules$parameters$directionType == 'CaseAboveCutoff', ]
    numberOfTopCaseFeatures <- nrow(topCaseFeatures)
    topCaseFeaturesIDX <- match(topCaseFeatures$feature, colnames(newDataInLoop))
    diagCaseMatrix <- newDataInLoop[, topCaseFeaturesIDX]
    caseRule <- combineRule1(diagCaseMatrix, method = 'case-specific', topCaseFeatures = topCaseFeatures, atLeastCase = unique(topCaseFeatures$atLeastNumber))
    
    topControlFeatures <- recursiveRules$parameters[recursiveRules$parameters$directionType == 'ControlAboveCutoff', ]
    numberOfTopControlFeatures <- nrow(topControlFeatures)
    topControlFeaturesIDX <- match(topControlFeatures$feature, colnames(newDataInLoop))
    diagControlMatrix <- newDataInLoop[, topControlFeaturesIDX]
    controlRule <- combineRule1(diagControlMatrix, method = 'control-specific', topControlFeatures = topControlFeatures, atLeastControl = unique(topControlFeatures$atLeastNumber))
    
    predictDF <- uniPredict(caseRule, controlRule)
    predictTable <- predictDF
    numberOfPredictable <- sum(predictTable$predict != 'unpredictable')
    numberOfUnPredictable <- sum(predictTable$predict == 'unpredictable')
    numberOfContradictory <- sum(predictTable$posControdictory == 1)
...
recursiveRules_cv <- function(dataFrame, 
  ...
  for(j in 1:R){
    ...
    for(i in 1:K){
      ...
      outTab <- data.frame(idx_test, pred$predictTable, stringsAsFactors = F)
...
recursiveRules_cv_para <- function(dataFrame, verbose = F, maxProcesses = 40, 
  ...
  for(j in 1:R){
    ...
    predAll <- foreach(i = 1:numberOfThreads, 
                       .export = c('sens', 'sens_low', 'spec', 'spec_low', 'ss_pre', 'combineRule1', 'surveyCandidates', 'uniPredict', 'recursiveRules_v1', 'predictRecurRules'), 
                       .combine = 'rbind') %dopar% {
...
tune_recurRules <- function(dataFrame, 
  ...
  performOut <- foreach(i = 1:numberOfThreads, 
                        .export = c('sens', 'sens_low', 'spec', 'spec_low', 'ss_pre', 'combineRule1', 'surveyCandidates', 'uniPredict', 'recursiveRules_v1', 'predictRecurRules', 'recursiveRules_cv'), 
                        .combine = 'rbind') %dopar% {
          ...
          for(each_atLeastNumber in atLeastNumber){
            for(each_maxRound in maxRound){
              ...
              if(performanceType == 'training'){
                ...
                predictableIDX <- which(predictInLoop$predictTable$predict != 'unpredictable')
                predictDF <- data.frame(response = response[match(predictInLoop$predictTable$sampleID[predictableIDX], rownames(dataFrame))],
                                        prediction = as.numeric(as.character(predictInLoop$predictTable$predict[predictableIDX])))
                ...
                predictDF <- data.frame(response = response[predictInLoop$predOut[[1]]$testIDX[predictableIDX]],
                                        prediction = as.numeric(as.character(predictInLoop$predOut[[1]]$ProbCase[predictableIDX])))
                numberOfPredicatable <- length(predictableIDX)
              }
