...
feature_filtering <- function(
  ...
  if(ks_test){
    pVecKStest <- apply(dataFrame[,-n], 2, function(x){
      ks.test(x[response==0|response=='control'], x[response==1|response=='case'])$p.value
    })
  ...
  if(wilcox_test){
    pVecWiltest <- apply(dataFrame[,-n], 2, function(x){
      wilcox.test(x[response==0|response=='control'], x[response==1|response=='case'])$p.value
    })
  ...
  if(t_test){
    pVecTtest <- apply(dataFrame[,-n], 2, function(x){
      t.test(x[response==0|response=='control'], x[response==1|response=='case'], var.equal = FALSE)$p.value
    })
  ...
  if(FC){
    ...
    foldChange_log <- apply(dataFrame[,-n], 2, function(x){
      log2(mean(x[response==1|response=='case'])/mean(x[response==0|response=='control']))
    })
  ...
  for(i in 1:R){
    if(is.null(main)){
      roc(response[fittedRF[[i]][,1]], fittedRF[[i]][,2], plot = T,
          main = round(roc(response[fittedRF[[i]][,1]], fittedRF[[i]][,2])$auc, 3),
          cex.main = cex.main, cex.lab = cex.lab, col = col)
    }
    else{
      roc(response[fittedRF[[i]][,1]], fittedRF[[i]][,2], plot = T,
          main = main,
...
rf_permute <- function(wholeDataframe, featureFilter = T, r=100){
  ...
  auc_orig <- roc(y, fit_orig$predOut[[1]][,2])$auc
  aucAll <- auc_orig
  ...
  for(i in 1:r){
    ...
    auc_perm <- roc(y_perm, fit_perm$predOut[[1]][,2])$auc
...
rf_permute_cv <- function(wholeDataframe, featureFilter = T, r=100){
  ...
  auc_ori <- roc(y[fitRFCV_ori$predOut[[1]][,1]], fitRFCV_ori$predOut[[1]][,2], plot = F)$auc
  ...
  for(i in 1:r){
    ...
    auc_perm <- roc(y_perm[fit_perm$predOut[[1]][,1]], fit_perm$predOut[[1]][,2], plot = F)$auc
    aucAll <- c(aucAll, auc_perm)
    cat(paste(i, sep = ','))
    cat('\n')
  }
