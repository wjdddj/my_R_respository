...
DF_average <- function(AllDF, replicateLabels, save.RData = F, filename, removeHighCV = F, CVcutoff = 0.2){
  sampleID <- replicateLabels
  uniqID <- unique(sampleID)
  AvgDF <- c()
  CVDF <- c()
  for(i in 1:length(uniqID)){
    replDF <- data.frame(t(AllDF[,sampleID==uniqID[i]]))
    if(nrow(replDF)==1) cat(paste('Not enough replicate for ', uniqID[i], '. CV will not be computed.\n', sep = ''))
    AvgDF <- rbind(AvgDF, apply(replDF, 2, mean, na.rm = T))
    CVDF <- rbind(CVDF, apply(replDF, 2, function(x){sd(x, na.rm = T)/abs(mean(x, na.rm = T))}))
  }
  AvgDF <- data.frame(t(AvgDF))
  CVDF <- data.frame(t(CVDF))
  rownames(AvgDF) <- rownames(AllDF)
  colnames(AvgDF) <- uniqID
  rownames(CVDF) <- rownames(AllDF)
  colnames(CVDF) <- uniqID
...
DF_split <- function(DataFrame, sample_table, splitBy){
...
CountInspection <- function(RawDataDF, general_stat, sample_table, cutoffCount = 500000, 
  ...
  # cat(ratio)
  LessThanCutoff <- totalcount_2000 < cutoffCount
