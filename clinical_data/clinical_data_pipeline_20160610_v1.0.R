...
surv_fit <- function(df_clinical_info, plot_covariate){
  ...
  fitCox <- coxph(Surv(time, censored) ~ ., data = df_clinical_info)
  ## KM plot just for 'followed'
  surv_formula <- as.formula(paste('Surv(time, censored) ~ ', plot_covariate, sep = ''))
...
################################################################################################################
kmplots <- function(
  surv_fit, main, display_coef_idx = NULL
){
  library(survival)
  ...
  ## start plotting
  par(mar = c(4,4,5,2))
  plot(fitSurv, col = line_colors[1:n_levels], lwd = 2, 
       xlab = 'Time (years)', ylab = 'Proportion Event Free', 
       main = main, 
       cex.main = 1, cex = 1, cex.lab = 1)
  legend('topright', 
         legend = c(paste(surv_strata, ' (n = ', fitSurv$n, ', event = ', fitSurv_sum$table[, 4], ')', sep = ''),
                    paste('HR = ', round(fitCox_sum$coef[rowid,2], 3), ', p-value =', round(fitCox_sum$coef[rowid,5], 3), sep = ''),
                    paste('log-rank test p-value =', round(fitCox_sum$sctest[3], 3), sep = '')),
...
get_demogr_tina <- function(dat, y_covariate, x_covariate, category_test=TRUE){
  ...
  if (length(x_num_idx)!=0) {
    ...
    ls_num_tables <- lapply(1:n_x_num, function(x_idx){
      ...
      num_mean <- tapply(x_num[,x_idx], y_cov, mean)
      num_sd <- tapply(x_num[,x_idx], y_cov, sd)
...
################################################################################################################
format_drug_line <- function(df_therapy_by_subject){
  ...
  df_therapy_by_subject <- data.frame(
    ...
    collectionPoint = 0,
    drugStartPoint = as.numeric(df_therapy_by_subject$drugstart - df_therapy_by_subject$collectiondate)/30,
    drugEndPoint = as.numeric(df_therapy_by_subject$drugend - df_therapy_by_subject$collectiondate)/30,
    drugDuration = as.numeric(df_therapy_by_subject$drugend - df_therapy_by_subject$drugstart)/30,
    determinationPoint = as.numeric(df_therapy_by_subject$determinationdate - df_therapy_by_subject$collectiondate)/30
  )
  ...
  df_therapy_by_subject$plotEndPoint <- ifelse(df_therapy_by_subject$unknownDuration == T,
                                               df_therapy_by_subject$drugStartPoint + 0.5,
...
################################################################################################################
## To compute match_score for a single case.
## input:
...
################################################################################################################
get_match_score <- function(received, recommend, wt_star, adjust_recommend = F, recom_weight = c(1, 0)){
...
##      as df_therapy. ls_recommend should contain columns, named 'durgRule', 'recom'.
##      3. adjust_recommend and recom_weight are inherited from get_match_score()
## output:
##      df_therapy. the same data.frame as input with a new column named 'match_scores' appended.
################################################################################################################
get_match_score_df <- function(df_therapy, ls_recommend, wt_star, adjust_recommend = T, recom_weight = c(1, 0)){
  ...
  match_scores <- llply(1:n_sample, function(i){
    ...
    recommend <- data.frame(druggroup = ls_recommend[[i]]$durgRule,
                            recommendation = ls_recommend[[i]]$recom, 
                            stringsAsFactors = F)
    get_match_score(received, recommend, wt_star, adjust_recommend = adjust_recommend, recom_weight = recom_weight)
  })
  df_therapy$match_score <- (unlist(match_scores) + 1)/2 ## recommendation score is between -1 and 1, scale to 0 and 1.
  df_therapy
}
...
##      3. caption, table name
##      4. outfile, output directory and filename 
################################################################################################################
get_demogr_ryan <- function(
  df_demographics, y_covariate, caption = caption,
  outfile = ''
){
  # caption <- 'my table'
  ...
  align <- paste0(c('ll', rep('c', ncol(x_table)-1)), collapse = '')
  print(xtable(x_table, align=align, caption = caption), include.rownames=FALSE, include.colnames=FALSE,
        hline.after=c(0, 2, nrow(x_table)), file = outfile, caption.placement = 'top', size = 'scriptsize')
