...
################################################################################################################
## main function for secondline unrestricted
## fit coxph model: surv ~ match_score + registry_case
################################################################################################################
main_per_weight <- function(df_therapy, ls_recommend, wt_star, reg_druggroup_mapping, adjust_recommend, recom_weight){
  #df_therapy <- df_therapy_2nd
  #wt_star <- data.frame(druggroup = colnames(wt_star_matrix),
  #                      weight = wt_star_matrix[1,])
  
  df_therapy_tnt <- get_therapy_tnt_df(df_therapy, start_line = 2, end_line = 3)
  df_therapy <- data.frame(df_therapy_tnt, 
                           registry_case = df_therapy$registry_case[match(df_therapy_tnt$masterdeid, df_therapy$masterdeid)])
  df_therapy$druggroups <- NULL
